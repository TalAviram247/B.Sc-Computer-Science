...
public class Editor1 {
	...
	public static void main (String[] args){
		...
		Color [][] img = Instush.read(filename);

		if(filter.equals("fh")){
			Instush.show(Instush.flippedHorizontally(img));
		}else if(filter.equals("fv")){
			Instush.show(Instush.flippedVertically(img));
		}else if(filter.equals("gs")){
			Instush.show(Instush.greyscaled(img));
		}
	}
}
